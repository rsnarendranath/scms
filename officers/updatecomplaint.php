...
if (strlen($_SESSION['alogin']) == 0) { 
  ...
  if (isset($_POST['update'])) {
    ...
    $sql3 = mysqli_query($con, "select * from tblcomplaints where complaintNumber = '$complaintnumber'");
    while ($row = mysqli_fetch_array($sql3)) {
      $rgdate = $row['regDate'];
      $noc = $row['noc'];
    }
    $sql2 = mysqli_query($con, "SELECT users.userEmail, tblcomplaints.complaintNumber FROM users INNER JOIN tblcomplaints ON users.id=tblcomplaints.userId where tblcomplaints.complaintNumber='" . $_GET['cid'] . "' GROUP by users.userEmail");
    while ($row = mysqli_fetch_array($sql2)) {
      $to = $row['userEmail'];

      require '../phpmailer/updatecmpmail.php';
    }
  }

...
if(isset($_POST['submit']))
{
	...
	$cityname=$_POST['city'];
	$id=intval($_GET['id']);
$sql=mysqli_query($con,"update city set sid='$province',cityName='$cityname',updationDate='$currentTime' where id='$id'");
			...
			<div class="container-fluid">
				...
				<div class="row">
					<div class="col-md-12">
						...
						<!-- <div class="row">
							<div class="col-md-10"> -->
								<p><a href="city.php"><< back to Add City</a></p>
...
<?php
$id=intval($_GET['id']);
$query=mysqli_query($con,"select state.id,state.stateName,city.cityName from city join state on state.id=city.sid where city.id='$id'");
while($row=mysqli_fetch_array($query))
{
?>	
											<div class="form-group">
												...
												<div class="col-sm-8">
													...
													<option value="<?php echo htmlentities($row['id']);?>"><?php echo htmlentities($proname=$row['stateName']);?></option>
													<?php $ret=mysqli_query($con,"select * from state");
													while($result=mysqli_fetch_array($ret))
													{
														$pro=$result['stateName'];
													if($proname=$pro)
													{
														continue;
													}
											...
											<div class="form-group">
												...
												<div class="col-sm-8">
													<input type="text" placeholder="Enter category Name"  name="city" value="<?php echo  htmlentities($row['cityName']);?>" class="form-control" required>
													<?php } ?>	
												</div>
											</div>
											<div class="hr-dashed"></div>
